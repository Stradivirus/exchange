...
interface CommoditiesIndexPostgreDto {
  date: string;
  dxy: number | null;
  vix: number | null;
}
...
interface Props {
  data: CommoditiesIndexPostgreDto[];
  onBack?: () => void;
}

const CommoditiesIndexPostgreSection: React.FC<Props> = ({ data, onBack }) => {
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px #0001', minWidth: 400 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <span style={{ fontWeight: 700, fontSize: '1.5rem' }}>지수 (PostgreSQL)</span>
        {onBack && (
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
          <thead>
            <tr style={{ background: '#f2f2f2' }}>
              ...
              <th style={{ padding: 8, border: '1px solid #eee' }}>DXY</th>
              <th style={{ padding: 8, border: '1px solid #eee' }}>VIX</th>
            </tr>
          </thead>
          <tbody>
            {data && data.length > 0 ? (
              ...
              <tr>
                <td colSpan={3} style={{ textAlign: 'center', padding: 16, color: '#888' }}>데이터 없음</td>
              </tr>
            )}
          </tbody>
        </table>
...
export default CommoditiesIndexPostgreSection;
