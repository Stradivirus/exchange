...
public class MainPageMongoService {
    ...
    // =================================================================================
    // 기존 기능: 메인 페이지용 최신 데이터 1건씩 조회 (변경 없음)
    // =================================================================================
    ...
    public List<GrainsDto> getLatest30GrainsDto() {
        ...
        // GrainsDto는 내부적으로 java.sql.Date를 사용하므로 변환 필요
    ...
    public List<StockDto> getLatest30StockDto() {
        ...
        // StockDto는 LocalDate를 사용 (ExchangeDto와 동일)
        return map.entrySet().stream()
                .sorted(Map.Entry.<LocalDate, StockDto.StockDtoBuilder>comparingByKey().reversed())
                .map(entry -> entry.getValue().date(entry.getKey()).build())
                .collect(Collectors.toList());
    }
    ...
    private ExchangeDto.ExchangeDtoBuilder getBuilder(Map<LocalDate, ExchangeDto.ExchangeDtoBuilder> map, LocalDate date) {
        return map.computeIfAbsent(date, d -> ExchangeDto.builder().date(d));
    }

    private List<ExchangeDto> buildAndSort(Map<LocalDate, ExchangeDto.ExchangeDtoBuilder> map) {
        return map.entrySet().stream()
