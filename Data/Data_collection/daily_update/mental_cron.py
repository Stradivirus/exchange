...
def main():
    print(f"=== 심리지수 업데이트 시작 ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}) ===")
    ...
    for stat_code, config in SENTIMENT_INDICATORS.items():
        ...
        if latest_doc:
            ...
            if start_dt > today:
                print(f"{config['indicator_name']}: 이미 최신 데이터임")
                continue
        ...
        for item_code in config['item_codes']:
            df = get_sentiment_data(stat_code, item_code, start_date_str, end_date_str, config['period'])
