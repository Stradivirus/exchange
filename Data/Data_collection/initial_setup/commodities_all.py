# initial_setup/commodities_all.py
"""
원자재 및 지수 데이터 전체 수집 (2010년~현재)
"""
import sys
sys.path.append('..')
...
from common import (
    ...
    COMMODITIES_INDICES, safe_clean_value, get_collection_stats
)


def main():
    """메인 실행"""
    print(f"=== 원자재/지수 데이터 전체 수집 시작 ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}) ===")
    ...
    for name, ticker in COMMODITIES_INDICES.items():
        try:
            ...
            if not data.empty:
                ...
                print(f"최신 가격: ${latest_value:,.2f} ({date_str})")
            else:
                print(f"{name}: 데이터 없음")
    ...
    # 통계 출력
    print("\n=== 저장된 데이터 확인 ===")
    for commodity_name in COMMODITIES_INDICES.keys():
        stats = get_collection_stats(client, commodity_name)
        print(f"{commodity_name}: {stats['total_count']:,}개")
